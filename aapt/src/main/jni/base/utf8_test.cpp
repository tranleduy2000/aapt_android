...
// up to the caller, not a base library).
    ...
    for (
    size_t i = 0;
    i < arraysize(kConvertRoundtripCases); ++i) {
    std::ostringstream utf8;
    utf8 <<
    WideToUTF8(kConvertRoundtripCases[i]);
    ...
    EXPECT_EQ(kConvertRoundtripCases[i], wide
    .
...
sizeof(multi16));
EXPECT_EQ(arraysize(multi16) - 1, multistring16.

length()

);
std::string expected;
memcpy(WriteInto(&expected, arraysize(multi)), multi,
sizeof(multi));
EXPECT_EQ(arraysize(multi) - 1, expected.
...
// >16 bits
EXPECT_EQ("\xF0\x90\x8C\x80",
SysWideToUTF8(kSysWideOldItalicLetterA)
);
...
EXPECT_EQ(std::string::npos, wide
.find(LR"(\\?\)"));
}
